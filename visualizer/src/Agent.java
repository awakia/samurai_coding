/**
 * This file is a part of software for SamurAI 2013 game visualizer.
 *
 * Takashi Chikayama disclaims to the extent authorized by law any and
 * all warranties, whether express or implied, including, without
 * limitation, any implied warranties of merchantability or fitness for a
 * particular purpose
 * 
 * You assume responsibility for selecting the software to achieve your
 * intended results, and for the results obtained from your use of the
 * software. You shall bear the entire risk as to the quality and the
 * performance of the software.
 */

package samurai2013;

/**
 * Describes an agent of the game
 * @author Takashi Chikayama
 */
class Agent {
    /** The team of the agent */
    int team;
    /** ID of the agent.  ID == 0 means a dog */
    int agnt;
    /**
     * Makes an agent structure with given team and ID.
     * @param t Team of the agent
     * @param a ID of the agent
     */
    public Agent(int t, int a) {
        team = t;
        agnt = a;
    }
    /**
     * Makes an agent structure reading the data in from an input stream.
     * @param is The input stream to read from
     * @throws Exception 
     */
    public Agent(LogStream is) throws Exception {
        this(is.nextInt(), is.nextInt());
    }
    
}
